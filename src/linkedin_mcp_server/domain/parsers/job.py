...
def parse_job_posting(html: str, *, include_raw: bool = False) -> JobPostingDetail:
    ...
    detail_div = s.find(attrs={"data-view-name": "job-detail-page"})
    if detail_div:
    ...
    if not job_id:
        ...
        link = s.find("a", href=JOB_VIEW_RE)
        if link:
            m = JOB_VIEW_RE.search(link["href"])
            if m:
                job_id = m.group(1)
    ...
    company_link = s.find("a", href=re.compile(r"/company/[^/]+/life/?"))
    if company_link:
        ...
        if not company:
            ...
            if p_tag:
                ...
                company = text(a_inner) if a_inner else text(p_tag)
        # Company URL
    ...
    verified_link = s.find("a", attrs={"href": "#"})
    if verified_link:
    ...
    if not title:
        ...
        title_p = s.find("p", class_=lambda c: c and "_0c38c653" in c)
        if title_p:
            title = text(title_p)
            if title:
                title = re.sub(r"\s*Verified job\s*$", "", title).strip()
    ...
    for bl in badge_links:
        badge_span = bl.find("span", class_=lambda c: c and "b043d390" in c)
        if badge_span:
            badge_text = text(badge_span)
    ...
    about_slot = s.find(
        "div",
        attrs={"data-sdui-component": lambda v: v and "aboutTheJob" in v},
    )
    if about_slot:
        # The expandable text box contains the full description
        text_box = about_slot.find("span", attrs={"data-testid": "expandable-text-box"})
        if text_box:
            description = text(text_box)
            ...
            if text(h2) == "About the job":
                ...
                if parent:
                    ...
                    if desc_p:
                        ...
                        description = text(text_box) if text_box else text(desc_p)
                break
